// Career guidance responses and questions database
export const careerResponses = {
...
Which field interests you most? I can provide more specific guidance!`
  ...
  general: {
    ...
    response: `I'm here to help you navigate your career journey! Here's a structured approach to career planning:
...
• **Job Description** - What does the role involve?
• **Required Skills** - What qualifications are needed?
• **Growth Prospects** - What are advancement opportunities?
• **Salary Range** - What's the earning potential?
• **Work Environment** - What's the typical workplace like?

💡 **My Recommendations:**
1. **Start broad, then narrow down** - Explore many options first
2. **Get hands-on experience** - Try internships or projects
3. **Network actively** - Connect with professionals
4. **Stay flexible** - Careers evolve, so keep learning

What specific area would you like to explore? I can provide detailed guidance on any career path!`
  },
...
export const getCareerResponse = (userMessage) => {
  ...
  if (message.includes('10th') || message.includes('tenth')) {
    return careerResponses.after10th.response;
  }
  
  if (message.includes('engineering') || message.includes('engineer')) {
    return careerResponses.engineering.response;
  }
  
  if (message.includes('medical') || message.includes('doctor') || message.includes('mbbs')) {
    return careerResponses.medical.response;
  }
  
  if (message.includes('commerce') || message.includes('business') || message.includes('ca') || message.includes('mba')) {
    return careerResponses.commerce.response;
  }
  
  if (message.includes('arts') || message.includes('humanities') || message.includes('psychology') || message.includes('law')) {
    return careerResponses.arts.response;
  }
  
  if (message.includes('skill') || message.includes('learn') || message.includes('develop')) {
    return careerResponses.skills.response;
  }
  
  // Default response for general career guidance
  return careerResponses.general.response;
};

// Function to get suggested follow-up questions
export const getFollowUpQuestions = (category) => {
  const followUpQuestions = {
    after10th: [
      "What are the career options in science stream?",
      "Which stream has better job opportunities?",
      "How to choose between PCM and PCB?",
      "What if I'm not sure about my interests?"
    ],
    engineering: [
      "Which engineering branch has the best salary?",
      "What are the job opportunities after engineering?",
    ...
    commerce: [
      "Which professional course should I choose?",
  ...
  return followUpQuestions[category] || followUpQuestions.general;
};
